...
def walk_dirs( top_path, action, quiet = False, only_these_dirs=None, except_these_dirs=None, only_these_files=None, except_these_files=None):
    """
    walk_dirs( path_to_search_into, callback_to_call_on_each_file, callback_is_quiet=False,
    ...
    for name in files:
        ...
        if stat.S_ISDIR( details.st_mode ):
            ...
            if only_these_dirs and os.path.basename( name ) in only_these_dirs:
                action( name, details, quiet )
                # don't descend further; action() has handled this dir
                continue
            elif except_these_dirs and os.path.basename( name ) in except_these_dirs:
                continue
            walk_dirs( name, action, quiet, only_these_dirs, except_these_dirs, only_these_files, except_these_files )
